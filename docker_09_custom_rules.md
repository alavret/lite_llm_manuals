...
Docker-версия гайда [09_custom_rules.md](09_custom_rules.md): берём ~266 regex-правил маскирования секретов и ПИД из open-source проекта [guardrails-llm-filter](https://github.com/cloud-ru-tech/guardrails-llm-filter) (Cloud.ru) и подключаем их к Presidio Analyzer из [docker_04_add_presidio.md](docker_04_add_presidio.md). Отличие от venv-версии: analyzer — официальный контейнер, своего `analyzer_server.py` нет, поэтому правила регистрируются не патчем кода, а inline-секцией `recognizer_registry` в `analyzer-config.yml` (с пересборкой образа).
...
GUI  →  LiteLLM :4000 (контейнер)
               →  Presidio Analyzer  :3000 (в сети Docker; найти PII + 250 новых правил)
               →  Presidio Anonymizer :3000 (в сети Docker; замаскировать — без изменений)
...
|---|---|---|
| `custom_recognizers.registry.yaml` (~85 КБ) | записи для `recognizer_registry.recognizers` (с `supported_languages:` списком) | **основной для Docker**: вставляются в `analyzer-config.yml` (п. 4) |
| `custom_recognizers.json` (~110 КБ) | ad-hoc формат LiteLLM/Presidio: `[{name, supported_language, supported_entity, patterns, context?}]` | альтернатива `presidio_ad_hoc_recognizers` (п. 6); в venv-варианте — основной (см. `09_custom_rules.md`) |
...
Пример записи из `custom_recognizers.registry.yaml`:
...
В Docker-варианте реестр распознавателей задаётся inline-секцией `recognizer_registry` в `analyzer-config.yml` (см. `docker_04_add_presidio.md`, раздел 2). Кастомные правила добавляются в тот же список.

1. Скопируйте `custom_recognizers.registry.yaml` в каталог сборки:

```bash
cp presidio_custom_rules/custom_recognizers.registry.yaml /opt/presidio/
```

2. В `/opt/presidio/analyzer-config.yml` допишите **в конец** списка `recognizers:` внутри секции `recognizer_registry:` все записи из `custom_recognizers.registry.yaml` (всё, что под его ключом `recognizers:` — 250 элементов вида):
...
recognizer_registry:
  ...
  recognizers:
    # ... предустановленные рекогнайзеры из default_recognizers.yaml
    # (CreditCardRecognizer, EmailRecognizer, IpRecognizer, ...) — БЕЗ ИЗМЕНЕНИЙ
    - name: credentials.url_with_creds
      supported_entity: URL_WITH_CREDS
      patterns:
        - name: credentials.url_with_creds
      ...
      supported_languages:
        - ru
    # ... ещё 249 записей из custom_recognizers.registry.yaml
```

Не удаляйте предустановленные записи: если секция `recognizers:` задана, Presidio грузит **только** то, что в ней перечислено.

3. Пересборка и проверка:

```bash
cd /opt/presidio && docker compose up -d --build
docker logs presidio-analyzer 2>&1 | tail -5
...
`127.0.0.1:5002` — порт, проброшенный на хост для ручных проверок (маппинг `127.0.0.1:5002:3000` из `docker_04`); LiteLLM внутри сети ходит на `presidio-analyzer:3000`.

Быстрая проверка нескольких новых сущностей:
...
В `/opt/litellm/config.yaml` добавьте в существующий `pii_entities_config` (guardrail `presidio-pii` из `docker_04`) все 247 строк из `pii_entities_config.snippet.yaml` (250 правил → 247 сущностей: `BEARER_TOKEN`, `API_KEY_HEADER` и `TOKEN` используются двумя правилами каждая):
...
Пересоздание (менялся `config.yaml`):
...
В провайдер должен уйти текст с `<DB_DSN>` и `<STRIPE_ACCESS_TOKEN>` (проверяется по логам из `docker_08`: `docker logs litellm`, либо DEBUG-уровнем из `docker_03`).

### 5.1. Конфликт с email_detector из гайда 02

Если стоит guardrail `email_detector` (`my_guardrails.py`) из `docker_02`, он срабатывает **раньше** presidio и заменяет email-подобные фрагменты на `[EMAIL REDACTED]`. В строках вида `postgres://admin:s3cretPw@db.corp.local:5432/prod` он съедает `s3cretPw@db.corp.local` — после этого regex `DB_DSN` (и `URL_WITH_CREDS`) уже не матчится: пароль скрыт, но пользователь и хост провайдеру видны.

Решение: убрать из `/opt/litellm/guardrails/my_guardrails.py` email-маскирование (`EMAIL_RE.sub("[EMAIL REDACTED]", ...)`), оставив SSN-блок. Email теперь маскирует presidio (`EMAIL_ADDRESS: MASK`) — с восстановлением значения клиенту через `output_parse_pii`, а `[EMAIL REDACTED]` терял данные безвозвратно. SSN-политика гайда 02 сохраняется.

Два контракта при правке `my_guardrails.py`:

- `apply_guardrail()` обязан **вернуть `inputs`** (dict), не `None` — иначе LiteLLM падает с `AttributeError: 'NoneType' object has no attribute 'get'` и клиент получает 400;
- guardrails примонтированы в контейнер как `./guardrails:/app/custom:ro` — достаточно отредактировать файл на хосте и `docker compose restart litellm`, пересборка не нужна.
...
Если пересобирать образ analyzer нельзя, тот же `custom_recognizers.json` подключается через LiteLLM — он будет отправлять правила в каждом запросе `/analyze`. Файл нужно смонтировать в контейнер LiteLLM (как в `docker_04`, раздел 7):

В `/opt/litellm/docker-compose.yml` сервиса `litellm`:

```yaml
    volumes:
      - ./config.yaml:/app/config.yaml:ro
      - ./custom_recognizers.json:/app/custom_recognizers.json:ro
```

В `/opt/litellm/config.yaml`:

```yaml
      presidio_ad_hoc_recognizers: /app/custom_recognizers.json
```

Минусы: ~110 КБ в **каждом** запросе к analyzer (LiteLLM ещё и чанкит большие тексты — каждый чанк понесёт полную пачку правил), плюс разбор 250 распознавателей на каждый запрос. Регистрация в реестре при сборке (п. 4) свободна от этого — используйте её как основной способ; ad-hoc оставьте для быстрых экспериментов с отдельными правилами.
...
Если какое-то пропущенное правило всё-таки нужно (например, `pii.fio-ru` при слабом NER) — верните его вручную: возьмите regex из исходного YAML, примените трансформации п. 2.2 (или попросите конвертер добавить override) и допишите запись в `analyzer-config.yml`.
...
| analyzer упал на старте (`Misconfigured engine` или YAML-ошибка) | записи вставлены с нарушением отступов, или удалены предустановленные записи из `recognizers:` — сравните с `docker_04` |
| правила не появились | образ не пересобран: нужен `docker compose up -d --build` |
| LiteLLM: `Presidio analyzer is not running` | analyzer не поднялся после правок — `docker logs presidio-analyzer` |
...
curl -sS http://127.0.0.1:5002/analyze -H 'Content-Type: application/json' \
  -d '{"text":"СНИЛС 112-233-445 95","language":"ru","entities":["SNILS"]}'
docker logs presidio-analyzer 2>&1 | tail -20
cd /opt/litellm && docker compose logs litellm --since 10m
```
...
(`artefacts/analyzer_server.py` — пример analyzer-сервера с загрузкой YAML при старте — в Docker-варианте не нужен: там официальный контейнер без патчей кода; он используется в venv-варианте, см. `09_custom_rules.md`.)

### 9.1 Процесс подключения

1. Допишите записи из `artefacts/custom_recognizers.yaml` (всё, что под ключом `recognizers:`) в **конец** списка `recognizer_registry.recognizers` в `/opt/presidio/analyzer-config.yml`, после предустановленных записей (см. п. 4).
2. Пересоберите: `cd /opt/presidio && docker compose up -d --build`.
3. Привязка к LiteLLM — без изменений по п. 5: все 247 сущностей из `artefacts/pii_entities_config.snippet.yaml` в `pii_entities_config`, затем `cd /opt/litellm && docker compose up -d`.
