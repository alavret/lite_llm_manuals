...
При `output_parse_pii: true` ([04d_add_presidio.md](04d_add_presidio.md)) ответ модели содержит те же плейсхолдеры — модель пишет `[EMAIL REDACTED]` буквально, если получила его на вход. Это видно прямо в ответе API без логов.
